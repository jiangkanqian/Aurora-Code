...
If you want to run the agent against an OpenAI-compatible model endpoint
(for example OpenAI, LiteLLM, vLLM, or similar `/v1/chat/completions` APIs),
set:
...
export ANTHROPIC_MODEL=gpt-4.1                        # or your deployment/model id
```
...
# 3) Start (Linux/macOS)
bash scripts/start-openai-compat.sh -p "hello"
```
...
powershell -ExecutionPolicy Bypass -File scripts/start-openai-compat.ps1 -p "hello"
```
